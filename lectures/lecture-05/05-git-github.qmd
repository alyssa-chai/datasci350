...
Chances are it is already there. Here is how to check from the command line:
...
To install or update Git for Windows (not WSL), check out [happygitwithr.com](https://happygitwithr.com/install-git.html)
:::

## Second step: introduce yourself to Git

:::{style="margin-top: 30px; font-size: 24px;"}
This is particularly important when you work with Git but without the GitHub overhead. The idea is to define how your commits are labelled, so others can identify your commits as coming from you
...
This opens a pager. Press `q` to get back to the prompt
...
While you are here, tell Git to call the first branch `main`. Without this, new repositories still start on a branch called `master`, and the commands later in this lecture will not work:
