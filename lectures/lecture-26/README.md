...
The last class before Quiz 05. We revise the four lectures the quiz covers, write two SQL queries together on the World Bank panel, and spend the end of the class on your project containers.
...
- SQL revision (Lecture 22): the evaluation-order ladder, `WHERE` against `HAVING`, `COUNT(*)` against `COUNT(value)`, and the three joins
- SQL practice: two exercises on `lectures/lecture-19/data/wdi_panel.parquet`, about twelve minutes of class time
...
The deck needs `duckdb`, and it reads `../lecture-19/data/wdi_panel.parquet`. Nothing calls the network.

## Before the next class

Quiz 05 is on Thursday 3 December and covers Lectures 21, 22, 23 and 25.

The final project is due on Tuesday 8 December at 11:59pm, on Canvas, and it must render inside a container. Test yours from a fresh clone before then. Appendix 03 lists the commands I run on your repository.

## Verification

Content aligned with the Fall 2026 decks for Lectures 21, 22, 23 and 25 on 13 September 2026. Query outputs produced with DuckDB 1.5.5 and pandas 3.0.3.
