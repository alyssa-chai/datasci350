...
- Cosine similarity: a dot product, spelled out in plain Python
...
- Our script spells the same formula out by hand, in a `cosine` function with a loop and `math.sqrt`, so nothing is hidden
...
def load_chunks(folder):
    """Split every file into paragraph chunks."""
    chunks = []
...
:::{.column width="45%"}
:::{style="margin-top: 20px;"}
- Ten lines of pre-processing: [paragraph chunking]{.alert} with an 80-character floor
- Each chunk travels as a pair, `(path.name, clean)`, so every retrieved passage [remembers which file it came from]{.alert}. That is what makes citations possible later
...
:::{.columns}
:::{.column width="60%"}
- Two small functions: one turns text into vectors, the other compares two vectors:
...
:::{.column width="40%"}
- `embed` sends a list of texts to `EMBED_MODEL` and gets back [one list of 768 numbers per text]{.alert}
- One call embeds [every chunk at once]{.alert}
- The question goes through the [same model]{.alert}
  - Two different embedding models produce two unrelated spaces, and the geometry stops meaning anything
- `cosine` is the formula from the earlier slide, written out: multiply the pairs, add them up, divide by the two lengths
- Dividing by the lengths is what makes the score sit between -1 and 1, whatever the passage length

:::{style="background: rgba(27, 58, 107, 0.08); padding: 12px; border-radius: 10px; font-size: 19px; margin-top: 10px;"}
The whole script is in [Appendix 02](#sec-appendix-02). A library would give you `cosine` in one call; here you can read every step of it
:::
:::
:::
:::
...
:::{.columns}
:::{.column width="55%"}
- Score every chunk against the question, then keep the best three:
...
:::{.column width="45%"}
- `QUESTION` is [the question we want to ask]{.alert}, a constant at the top of the file. Edit it and run the script again
- The first loop takes the [text half]{.alert} of each pair, because `embed` wants plain strings
- The second loop produces [one score per chunk]{.alert}, in the same order as `chunks`
- `ranked` holds `(score, chunk)` pairs. Sorting a list of pairs sorts on the [first item]{.alert}, so `reverse=True` puts the best score first
- `[:TOP_K]` keeps the three highest
- [Commercial "vector search" is this, at a much larger scale]{.alert}

:::{style="background: rgba(27, 58, 107, 0.08); padding: 12px; border-radius: 10px; font-size: 19px; margin-top: 10px;"}
The retrieval half of RAG is [these two slides]{.alert}. Everything after them is prompting
:::
:::
:::
:::
...
- The loop takes the [text half]{.alert} of each retrieved pair, and `join` glues the three passages together with blank lines between them
...
- Point `CHAT_MODEL` at any model you have pulled. Only the writing changes; retrieval stays the same
...
4. Open `rag.py`, set `QUESTION` to "What does Quiz 02 cover?", and run `python rag.py`
...
- The pure-Python loop is fine for a few hundred chunks. At [100,000 chunks]{.alert} it would keep you waiting on every question
...
- Cosine similarity [ranks candidates]{.alert}: one score per chunk, sorted
...
- Ninety lines of plain Python is a [working pipeline]{.alert}. Databases and frameworks run the same five stages at a larger scale
:::
...
The package is not installed in the Python you are running. `pip install ollama`, and check `which python3` if you use environments.
