...
- [DuckDB]{.alert}: SQL over files, out-of-core, results back into pandas
- Our little [benchmark]{.alert}: [DuckDB](https://duckdb.org/) finished in under a second, way ahead of [pandas](https://pandas.pydata.org/) and [Dask](https://dask.org/)
- pandas for small data, DuckDB when it grows, Dask when it leaves one machine
...
:::{.columns}
:::{.column width="50%"}
- Your code works, it is fast thanks to DuckDB, the report renders, the repository is documented
...
charset_normalizer-3.5.1 idna-3.19 polars-1.44.2
polars-runtime-32-1.44.2 requests-2.34.2
urllib3-2.7.0
```
:::
...
- Eight packages resolved in [378 milliseconds]{.alert}, environment created along the way
:::
:::
:::
...
- Mine is 182 lines for eight packages. [You never edit it by hand]{.alert}
...
:::{.columns}
:::{.column width="50%"}
- [Dependency management]{.alert}: writing down the software your code assumes
- Python 2 `print`: the script was fine, the environment moved
- [`venv` and `pip`]{.alert}: `pip freeze` writes pins, `pip install -r` reads them
- [Conda]{.alert}: `--from-history` travels, [build strings]{.alert} stay home
:::

:::{.column width="50%"}
- [uv]{.alert}: `pyproject.toml` + `uv.lock`, then `uv sync`
- [Containers]{.alert}: image = recipe, container = running instance, registry = storage
- [Layer caching]{.alert}: two-minute build, 0.754-second rebuild
- Docker Desktop is free; [Podman, Colima, OrbStack]{.alert} run the same commands
:::
:::

:::{style="background: rgba(27, 58, 107, 0.08); padding: 15px; border-radius: 10px; font-size: 20px; margin-top: 15px;"}
Your container is the deliverable: I run `docker build` and `docker run`, and your report appears or it does not 😅
:::
:::
