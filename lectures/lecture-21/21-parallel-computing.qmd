...
- Tasks are slow for two reasons: [the processor is busy, or idle while waiting for an answer]{.alert}
- [CPU-bound]{.alert}: processor busy, so only more processors help. Use [`ProcessPoolExecutor`](https://docs.python.org/3/library/concurrent.futures.html#concurrent.futures.ProcessPoolExecutor) (works like `joblib`)
- [I/O-bound]{.alert}: processor idle, waiting for a web server, disk or database. Use [`ThreadPoolExecutor`](https://docs.python.org/3/library/concurrent.futures.html#concurrent.futures.ThreadPoolExecutor)
- Both copy `map`: a function and an iterable
...
:::{.column width="50%"}
- Waiting needs no second processor: cheap threads, no new processes
...
- Lecture 19 downloaded World Bank indicators one country at a time
- Same `get_wdi()` pattern, now with all three requests at once
:::
:::
:::
...
:::{.columns}
:::{.column width="50%"}
- [Big O notation](https://en.wikipedia.org/wiki/Big_O_notation) describes how the runtime grows as the input grows
...
- Four cores: close to 4× faster, minus coordination costs
- Best candidates: [embarrassingly parallel O(n) tasks]{.alert}
- For example, `n` independent calculations, or one function applied to `n` separate inputs
- They share no state and do not depend on each other

:::{style="background: rgba(230, 57, 70, 0.1); padding: 15px; border-radius: 10px; font-size: 19px; margin-top: 20px;"}
Parallelism never changes the Big O class, only divides the constant in front. O(n²) on eight cores is still O(n²); you just hit "too slow" a little later
:::
:::
...
- `n = 100_000` = 10 billion operations; no core count rescues that
- Also poor candidates: steps that depend on earlier results, shared state, problems limited by memory rather than CPU
:::
:::
:::
...
:::{.columns}
:::{.column width="50%"}
- Some parts of a program [cannot run in parallel]{.alert}: setup, merging results, steps that wait for the previous one
- That part is the [serial share]{.alert} $s$; the rest, $p = 1 - s$, can be split across cores
- More cores only speed up $p$, so $s$ sets a ceiling ([Amdahl's law](https://en.wikipedia.org/wiki/Amdahl%27s_law))
...
- Even with infinite cores, the serial 10% remains: at most $1/0.1 = 10\times$
:::
...
:::{.columns}
:::{.column width="50%"}
- CPython (the Python you run) has a [Global Interpreter Lock (GIL)](https://en.wikipedia.org/wiki/Global_Interpreter_Lock)
- Only [one thread]{.alert} runs Python code at a time, whatever your core count
- Threads help while code [waits]{.alert}: the lock is released during downloads and API calls
- Threads do not help while code [computes]{.alert}: the lock keeps other threads idle
- CPU-bound work needs [processes]{.alert}: each is a separate Python with its own GIL
- [NumPy](https://numpy.org/), [Polars](https://polars.dev/) and [DuckDB](https://duckdb.org/) [release the GIL]{.alert} in their C/Rust code, so they use all cores without processes
- A [free-threaded build]{.alert} (no GIL) is official since Python 3.14 ([PEP 779](https://peps.python.org/pep-0779/)), but not the default
:::

:::{.column width="50%"}
- How the GIL maps onto tools you know:
...
:::{.columns}
:::{.column width="52%"}
- [Dask](https://dask.org/): a parallel computing library for Python
- Scales today's ideas to [data bigger than RAM](https://docs.dask.org/en/stable/best-practices.html){.alert} and [clusters](https://docs.dask.org/en/stable/deploying.html) of many machines
- You write [NumPy](https://numpy.org/)/[pandas](https://pandas.pydata.org/)-style code; Dask handles the parallelism
- [Task scheduler](https://docs.dask.org/en/stable/scheduling.html){.alert}: splits work into a graph of small tasks across your cores
- Collections: [arrays](https://docs.dask.org/en/stable/array.html){.alert}, [DataFrames](https://docs.dask.org/en/stable/dataframe.html){.alert} and [lists](https://docs.dask.org/en/stable/bag.html){.alert} that behave like the ones you know
- Actively developed, with releases most months ([2026.8.0](https://github.com/dask/dask/releases/tag/2026.8.0), August 2026)
- Next lecture: Dask or a faster single-machine engine?
:::
...
`joblib` splits your functions, not your data. [Dask chunks the data itself,]{.alert} so it can be larger than your RAM
:::
:::
:::
:::
...
:::{.columns}
:::{.column width="52%"}
- `dask.distributed.Client()` opens a diagnostics dashboard on port 8787
- Right: our three-stage pipeline, run in a loop to fill the picture
- Row = one worker thread; coloured bar = one task
- Colours: `generate_data`, `transform_data`, `aggregate_data`
- Bars stacked vertically ran at the same moment
- Fastest way to check whether your work is actually parallel

:::{style="background: rgba(27, 58, 107, 0.08); padding: 15px; border-radius: 10px; font-size: 19px; margin-top: 15px;"}
White space = idle worker. Large gaps usually mean chunks are too big: fewer tasks than workers
:::
:::
...
:::{.columns}
:::{.column width="50%"}
- [Measure first]{.alert}: `%timeit` and `cProfile` find the slow part; Amdahl's law says what fixing it is worth
- [Ask two questions]{.alert}: are tasks independent? Is each heavy enough to justify a worker? Any "no" means don't parallelise
- [Pick the right algorithm]{.alert}: $O(n \log n)$ beats parallel $O(n^2)$ on any real input
- [Pick the right engine]{.alert}: DuckDB or Polars may beat parallel pandas (next lecture)
- [Start small]{.alert}: NumPy or pandas may already have a fast built-in
- [Sample first]{.alert}: do you really need all those terabytes?
:::

:::{.column width="50%"}
- [Switch to parquet]{.alert}: the format alone gave a large speed-up earlier
- [Call `.compute()` and `.persist()` sparingly]{.alert}: each call runs the graph, so batch work into one
- [Experiment with chunk sizes]{.alert}: try a few, or start from `chunks='auto'`
- More tips: [Dask best practices](https://docs.dask.org/en/latest/best-practices.html)

:::{style="background: rgba(230, 57, 70, 0.1); padding: 15px; border-radius: 10px; font-size: 19px; margin-top: 15px;"}
[Most common mistake]{.alert}: parallelising before profiling. If the bottleneck is disk reads or an $O(n^2)$ loop, more cores won't help, and you'll lose an afternoon finding out
:::
:::
:::
:::
...
:::{.columns}
:::{.column width="50%"}
- [Serial and parallel execution]{.alert}: `map`, embarrassingly parallel problems, `joblib`
- [`concurrent.futures`]{.alert}: processes for CPU work, threads for waiting
- [Big O notation]{.alert}: parallelism cuts the constant, not the class
- [Amdahl's law]{.alert}: the serial share caps the speed-up
- [The GIL]{.alert}: threads for waiting, processes for computing; NumPy sidesteps both
:::

:::{.column width="50%"}
- [Dask]{.alert}: lazy arrays and DataFrames for data bigger than RAM
- [`dask.delayed`]{.alert}: parallelises your own functions; `dask.visualize` shows the plan
- [Parquet]{.alert}: January 2000 data from 182 MB to 86 MB, much faster group-by
- Next lecture, [Scaling Data Analysis in Practice]{.alert}: Polars and DuckDB vs pandas and Dask on the WDI panel
- Polars and DuckDB use every core by default, no configuration
...
- SQL on DataFrames before then: [`tutorials/04-duckdb-sql-tutorial.qmd`](https://danilofreire.github.io/datasci350/04-duckdb-sql-tutorial.html)
:::
:::
:::
