...
<https://ollama.com/>
:::
Everything so far has been [typed at a prompt]{.alert}. Today it becomes something [your code calls]{.alert}, the difference between one question and ten thousand
:::
:::
:::
...
- The reply has a fixed shape, so your code can pull out the piece it wants
- [Every chat window has an API underneath it,]{.alert} and so does every app on your phone
- Today you [use]{.alert} an API. [Lecture 18]{.alert} opens one up properly: URLs, status codes, and the `requests` library for your final project's data
:::
...
:::{.columns}
:::{.column width="50%"}
- Installing Ollama in lecture 12 gave you a command and started [a small web server]{.alert}
...
- You own [both ends]{.alert}, so when something breaks, nobody else's server is to blame
:::
:::
:::
...
JSON arrives as one long line; the machine reading it does not need newlines. Pipe it through a formatter to read it:
...
- A browser would [render]{.alert} that response into a page. `curl` shows the [raw text]{.alert}, which is what your Python receives
- `-s` hides the progress meter. `|` is the [pipe]{.alert} from lecture 04, feeding `curl`'s output into the next command
- `python3 -m` runs a [module]{.alert} that ships with Python instead of a file of your own. `json.tool` indents JSON
...
The numbers `ollama show` printed in lecture 12, now in a form your code can read.

- `size` is in bytes. 1,321,098,329 is the 1.3 GB you downloaded
- `digest` is a [sha256 fingerprint]{.alert} of the exact weights. Matching digests mean the identical model, which is how you pin one in a paper
...
- `/v1/chat/completions`, last in the table, is the one your Python uses
:::
:::
:::
:::
...
- [The conversation is resent every time]{.alert}. The API has no memory, so continuing a conversation means sending the whole `messages` list again
- Turn 20 pays for turn 1 for the twentieth time. Long chats cost much more per reply, which is why agents get expensive
- Prices are quoted per million tokens, input and output listed separately on the model's page
...
3. Give it a name and set the spending limit to [$0.00]{.alert}. People skip this step. At $0.00 a mistake in a loop costs you nothing, because the request is refused rather than billed
...
- Everything below these five lines is [identical]{.alert}: same client, same `messages`, same `response.choices[0].message.content`
- Keep the address, the key and the model id in [named constants at the top]{.alert} of the file. Switching backends is then a two-line edit
...
- Today's exercise uses 15 rows: one local run and one hosted run stay inside the daily 50
:::
:::
:::
:::
...
- I wrote those labels by hand. That column is the [answer key]{.alert}, the only reason we can say anything about accuracy at the end
...
- A regular expression works until the model writes "I would lean bearish", hedges, or answers in a different order
...
:::{.columns}
:::{.column width="55%"}
Wrap the call in a function and feed it a column. The whole thing is [demo/classify.py](https://github.com/danilofreire/datasci350/blob/main/lectures/lecture-14/demo/classify.py), reading [demo/headlines.csv](https://github.com/danilofreire/datasci350/blob/main/lectures/lecture-14/demo/headlines.csv):
...
- The loop walks the column one headline at a time, with a network call inside it
- `json.loads` turns each reply into a dictionary, so `["sentiment"]` reaches the label
- `BASE_URL`, `API_KEY` and `MODEL` sit at the top of `classify.py`. Those three lines are all you edit to swap servers
- The result lands as a [column beside the data]{.alert}, where pandas can work on it
...
- The errors are [not random]{.alert}. They sit in one class, and a single accuracy number hides that
...
`diff` prints only the lines that differ, so silence is the good outcome. Silence also looks like a broken command, so ask for something you can see:
...
- You are editing [three constants]{.alert} at the top of the file and nothing else
- If you find yourself changing the `classify` function, stop and re-read the swap slide
...
An agent is [the call you just made]{.alert}, put in a loop, and given permission to touch your computer. The rest is ordinary Python around it:
...
[Step 3]{.alert} is a real action on your real computer. That is what makes an agent feel like magic and what makes one dangerous
:::

:::{.column width="45%"}
:::{style="font-size: 21px;"}
- A chatbot [suggests]{.alert}. You copy, you paste, you fix the indentation
- An agent [acts]{.alert}, looks at what happened, and acts again. The loop can run for a hundred turns without asking you anything
...
- `json.loads` gives you `tool_arguments` as a dictionary. That is [structured output again]{.alert}, from part 3, now doing real work
- Remember `"capabilities": ["completion", "tools"]` in your own model's JSON? That is what it was telling you
- `messages` grows on every turn, and the whole list is resent. A long agent session therefore costs much more than a short one
:::
:::
:::
:::
...
Take one leg away and the attack stops working
:::
...
- Which two it missed, both the same mistake, showed up only by [reading the output against the labels]{.alert}
...
Today your model answered from what it learned in training. Next class it answers from [documents you give it]{.alert}, material it has never seen
...
:::{.column width="48%"}
:::{style="font-size: 23px;"}
- `api_key="ollama"` is not a secret and not a password
- The library refuses to start without one, so Ollama accepts any string and throws it away
- A required argument that does nothing looks like a bug. It is a compatibility shim, and the reason one client library can talk to both backends
